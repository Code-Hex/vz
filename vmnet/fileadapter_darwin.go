...
type PacketForwarder[T io.Closer] interface {
	...
	WritePacketsToConn(conn T, packetCount int) (int, error)
	...
	WritePacketsToInterface(iface *Interface, packetCount int) (int, error)
}
	...
	start = sync.OnceFunc(func() {
		...
		packetAvailableEventCallback := func(estimatedCount int) {
			for estimatedCount > 0 {
				...
				if _, err := forwarder.WritePacketsToConn(conn, packetCount); err != nil {
		...
		for {
			...
			if writtenCount, err := forwarder.WritePacketsToInterface(iface, packetCount); err != nil {
				reportError(err, fmt.Sprintf("forwarder.WritePacketsToInterface failed with packetCount=%d, writtenCount=%d", packetCount, writtenCount))
				break
			}
		}
