...
func (f *StreamPacketForwarder) WritePacketsToConn(conn net.Conn, packetCount int) (int, error) {
	_, err := f.readPktDescsManager.writePacketsToConn(conn, packetCount)
	if err != nil {
		return 0, err
	}
...
func (f *StreamPacketForwarder) WritePacketsToInterface(iface *Interface, packetCount int) (int, error) {
...
func (v *pktDescsManager) writePacketsToConn(conn net.Conn, packetCount int) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("buffersForWritingToConn failed: %w", err)
	}
	n, err := buffers.WriteTo(conn)
	if err != nil {
		return n, fmt.Errorf("buffers.WriteTo failed: %w", err)
	}
	return n, nil
}
