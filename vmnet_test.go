...
func TestVmnetSharedModeAllowsCommunicationBetweenMultipleVMs(t *testing.T) {
	...
	if network, err := vmnet.NewNetworkWithSerialization(serialization); err != nil {
		...
		t.Fatal(err)
	} else if file3, start3, _, err = vmnet.DatagramFileAdaptorForInterface(t.Context(), iface); err != nil {
		t.Fatal(err)
	}
	startFuncsWG.Add(1)
	go func() {
	...
	if network4, err := vmnet.NewNetworkWithSerialization(serialization); err != nil {
		...
		t.Fatal(err)
	} else if file4, start4, _, err = vmnet.DatagramNextFileAdaptorForInterface(t.Context(), iface); err != nil {
		t.Fatal(err)
	}
	startFuncsWG.Add(1)
	go func() {
